...
import slackDigests from "@/data/slack_digests.json";
...
// -------------------------------------------------------------------
// buildSlackDigest — Davide's #1 unsolved ask: one AI read across a
// title's several Slack channels (product, creators, community, vendor)
// so he can self-serve a title's status without pinging the team.
//
// Build-time pattern: the narrative (headline, synthesis, per-channel
// summaries, "needs you" list) is authored by Claude and committed in
// slack_digests.json. Here we re-derive the *hard* numbers — channel
// count, message count, flagged count, latest activity — from the live
// slackMessages so the counts can never drift from the authored prose.
// Returns null for titles with no Slack footprint.
// -------------------------------------------------------------------
export function buildSlackDigest(titleId) {
  const msgs = getSlackForTitle(titleId);
  if (msgs.length === 0) return null;

  const authored = slackDigests[titleId] || {};
  const channelSummaries = authored.channel_summaries || {};

  // Group the real messages by channel for truthful per-channel stats.
  const byChannel = new Map();
  for (const m of msgs) {
    const g = byChannel.get(m.channel) || {
      channel: m.channel,
      messageCount: 0,
      flaggedCount: 0,
      latest: "",
    ...
    g.messageCount += 1;
    if (m.flagged_for_action) g.flaggedCount += 1;
    if ((m.timestamp || "") > g.latest) g.latest = m.timestamp || "";
    if (m.author) g.authors.add(m.author);
    byChannel.set(m.channel, g);
  }

  const channels = Array.from(byChannel.values())
    .map((g) => {
      const meta = channelSummaries[g.channel] || {};
      return {
        channel: g.channel,
    ...
    // flagged channels first, then most recent activity.
    .sort((a, b) => {
  ...
  return {
    titleId,
    asOf: authored.as_of || null,
    headline: authored.headline || null,
    synthesis: authored.synthesis || null,
    needsYou: authored.needs_you || [],
    channels,
    channelCount: channels.length,
    messageCount: msgs.length,
    flaggedCount: msgs.filter((m) => m.flagged_for_action).length,
  };
}
...
// -------------------------------------------------------------------
// Every needs-attention item must trace to a tracked Jira ticket. For a
// beat-level blocker that means the governing ticket: the open item on
// the beat that is actually being held — preferring one that carries a
// blocker chain (is_blocked_by), then by priority.
function _governingTicketForBeat(beatId) {
  if (!beatId) return null;
  const onBeat = tickets.filter((t) => t.beat_id === beatId);
  if (onBeat.length === 0) return null;
  const open = onBeat.filter((t) => !isTicketDone(t));
  const pool = open.length ? open : onBeat;
  const rank = { P0: 0, P1: 1, P2: 2 };
  const sorted = [...pool].sort((a, b) => {
    const ab = a.is_blocked_by && a.is_blocked_by.length ? 0 : 1;
    const bb = b.is_blocked_by && b.is_blocked_by.length ? 0 : 1;
    if (ab !== bb) return ab - bb; // held (blocked) items first
    return (rank[a.priority] ?? 3) - (rank[b.priority] ?? 3); // then priority
  });
  return sorted[0].ticket_id;
}
    ...
    if (beat.lifecycle_stage === "Asset Lock" && beat.lifecycle_stage_entered_at) {
      ...
      if (daysStuck >= 2) {
        items.push({
          ...
          type: "risk",
          priority: "P0",
          // Name the actual blocker, not the generic lifecycle stage.
          headline: beat.beat_name,
          detail: `${
            beat.blocker_summary || "Held — needs unblocking"
          } · ${daysStuck}d held`,
          title_id: beat.title_id,
          ticketId: _governingTicketForBeat(beat.beat_id),
        });
      }
    }
  }
  ...
  for (const d of (standup.pending_decisions || [])) {
    if (d.decision_date <= DEMO_TODAY_ISO) {
      ...
      items.push({
        ...
        title_id: d.title_id,
        ticketId: d.linked_ticket_id || null,
      });
    }
  }
  ...
  for (const beat of beats) {
    if (
      ...
      items.push({
        ...
        headline: beat.beat_name,
        detail: beat.blocker_summary || beat.status.replace(/_/g, " "),
        title_id: beat.title_id,
        ticketId: _governingTicketForBeat(beat.beat_id),
      });
    }
...
// Link straight to the actual Jira ticket. Returns null for ids that don't
// resolve to a real ticket (e.g. proposed/extracted tickets not yet created),
// so only genuine tickets become clickable. Render with target="_blank" so a
// Jira key shown anywhere in the app opens the ticket in Jira.
export function jiraTicketHref(ticketId) {
  if (!ticketId) return null;
  if (!getTicket(ticketId)) return null;
  return jiraUrl(ticketId);
}
