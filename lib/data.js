...
// Deep-link straight to a ticket on the portfolio board. The board reads
// ?focus= on mount and auto-opens the ticket drawer, so any Jira key shown
// anywhere in the app can become a one-click jump to the ticket itself.
export function ticketDeepLink(ticketId) {
  if (!ticketId) return null;
  if (!getTicket(ticketId)) return null;
  return `/tickets?focus=${encodeURIComponent(ticketId)}`;
}
